# Capability: Portal Premium Experience

## Purpose
The enterprise-grade, sales-ready visual and interaction standards for the authenticated portals (`/owner`, `/employee`, `/client`). Governed by a documented Premium Quality Bar (PQB) in `frontend/portal-kit/PREMIUM_QUALITY_BAR.md` and enforced in CI, this capability ensures that all customer and operator surfaces meet Stripe-minimal visual hierarchy, responsive budgets, accessible interaction, and delightful, resilient UX.
