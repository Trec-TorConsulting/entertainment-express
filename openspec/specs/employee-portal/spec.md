...
### Requirement: How We Reach You
Staff SHALL set SMS/WhatsApp/push opt-in and quiet hours on `/employee/me` (profile). Crew SHALL NOT edit company message templates.

#### Scenario: Crew cannot edit templates
- **WHEN** an `EE Crew` user saves a company notification template
- **THEN** access is denied
...
### Requirement: Worker Earning Transparency Hub
The system SHALL present workers with an itemized earnings statement for each completed event in `/employee/timesheets` and `/employee/earnings`, detailing base pay, overtime, commission line items, and their share of client tips.

#### Scenario: Worker views event pay slip
- **WHEN** a crew member views a completed event in the employee portal
- **THEN** they see an itemized breakdown showing gig base fee ($350), 1 hour overtime ($45), and tip allocation ($60) totaling $455
